...
//  https://www.cnblogs.com/turling/p/11667441.html
...
func main() {
	...
	for v := 0; v < valueOfNd.NumField(); v++ {
		fieldValue := valueOfNd.Field(v)
	...
	fmt.Println("c addr", valueOfC)
	// 取出c地址的元素(c的值)
	valueOfC = valueOfC.Elem()
	if valueOfC.CanAddr() {
		fmt.Println("c kind", valueOfC.Kind())
	}
	fmt.Println("CanSet", valueOfC.CanSet())
	// 修改c的值为1
	valueOfC.SetInt(1)
	// 打印c的值
	fmt.Println("valueOfC",valueOfC.Int())
