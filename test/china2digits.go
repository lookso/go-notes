...
var chineseCountingString = map[string]int{"十": 10, "百": 100, "千": 1000, "万": 10000, "亿": 100000000, "兆": 1000000000000, "拾": 10,
	"佰": 100, "仟": 1000}

// 中文转阿拉伯数字
var chineseCharNumberDict = map[string]int{"幺": 1, "零": 0, "一": 1, "二": 2, "两": 2, "三": 3, "四": 4, "五": 5,
	"六": 6, "七": 7, "八": 8, "九": 9, "十": 10, "百": 100,
	"千": 1000, "万": 10000, "亿": 100000000, "兆": 1000000000000, "壹": 1, "贰": 2, "叁": 3, "肆": 4, "伍": 5, "陆": 6, "柒": 7, "捌": 8, "玖": 9, "拾": 10,
	"佰": 100, "仟": 1000}
...
func convertDigitsStringToFloat(digitsString string) float64 {
	...
	for i := 0; i < len(CHINESE_PER_COUNTING_STRING_LIST); i++ {
		if strings.Contains(digitsString, CHINESE_PER_COUNTING_STRING_LIST[i]) {
			value, exists := CHINESE_PER_COUNTING_DICT[string(CHINESE_PER_COUNTING_STRING_LIST[i])] //百分
			if exists {
	...
	if err != nil {
		...
		fmt.Println("perCountingString", perCountingString, floatResult)
...
func ChineseToDigits(chineseCharsToTrans string, percentConvert bool, simpilfy interface{}) string {
	fmt.Println("chineseCharsToTrans-init", chineseCharsToTrans)
	//"""
	//汉字数字切割 然后再进行识别
	//"""
	...
	if chPartString != "" {
		fmt.Println("chineseCharsToTrans-init-111", chineseCharsToTrans, chPartString)
		tag := true
		for i := 0; i < len(CHINESE_PER_COUNTING_STRING_LIST); i++ {
		...
		fmt.Println("chineseCharsToTrans-init-222", chineseCharsToTrans)
		chineseChars := []rune(chineseCharsToTrans)
		...
		chineseChars = []rune(chineseCharsToTrans)
		perCountingString := ""
		fmt.Println("before-111-chineseCharsToTrans", chineseCharsToTrans)
		for i := 0; i < len(CHINESE_PER_COUNTING_STRING_LIST); i++ {
			...
			if strings.Contains(chineseCharsToTrans, CHINESE_PER_COUNTING_STRING_LIST[i]) {
				...
				fmt.Println("before-chineseCharsToTrans", chineseCharsToTrans)
				chineseCharsToTrans = strings.Replace(chineseCharsToTrans, CHINESE_PER_COUNTING_STRING_LIST[i], "", -1)
				fmt.Println("chineseCharsToTrans", chineseCharsToTrans)
			}
		}
		...
		floatResult, err := strconv.ParseFloat(convertResult, 32)
		fmt.Println("floatResult", floatResult)
		fmt.Println("--------------------")
		if err != nil {
			panic(err)
		} else {
			if percentConvert == true {
				...
				fmt.Println("perCountingString", perCountingString)
				switch perCountingString {
				case "%":
...
var CHINESE_PURE_COUNTING_UNIT_LIST = [6]string{"十", "百", "千", "万", "亿", "兆"}

var TRADITIONAl_CONVERT_DICT = map[string]string{"壹": "一", "贰": "二", "叁": "三", "肆": "四", "伍": "五", "陆": "六", "柒": "七",
	"捌": "八", "玖": "九"}
var SPECIAL_TRADITIONAl_COUNTING_UNIT_CHAR_DICT = map[string]string{"拾": "十", "佰": "百", "仟": "千", "萬": "万", "億": "亿", "兆": "兆"}
...
func standardChNumberConvert(chNumberString string) string {
	...
	fmt.Println("chNumberStringList", chNumberStringList)
	// #大于2的长度字符串才有检测和补位的必要
...
func TakeChineseNumberFromString(chTextString string, opt ...interface{}) interface{} {
	...
	if len(CHNumberStringList) > 0 {
		for i := 0; i < len(CHNumberStringList); i++ {
			fmt.Println("baifenzhi-", CHNumberStringList[i], percentConvert, simpilfy)
			tempCHToDigitsResult = ChineseToDigits(CHNumberStringList[i], percentConvert, simpilfy)
			fmt.Println("tempCHToDigitsResult", tempCHToDigitsResult)
...
func main() {
	fmt.Println(TakeChineseNumberFromString("十-百分之十-百分之五十八-百分之五", true, true, true))
